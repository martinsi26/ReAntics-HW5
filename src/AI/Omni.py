...
class AIPlayer(Player):
    ...
    def __init__(self, inputPlayerId):
        super(AIPlayer,self).__init__(inputPlayerId, "Omni_HW5_AI_rhoades26_martinsi26")
    ...
    def utility(self, currentState, preCarrying):
        ...
        if TRAIN:
            inputs = 0
            inputs += 4               # 2 queens
            inputs += 4 * 100 * 2 * 2 # 4 types, 100 board spots, x and y, 2 players
            inputs += 11 * 2 * 2      # 11 objects to place, 2 players, 2 coordinates per object

            params = [0 for _ in range(inputs)]

            my_inventory = getCurrPlayerInventory(currentState)
            myant1 = getAntList(currentState, currentState.whoseTurn, (QUEEN,))
            myant2 = getAntList(currentState, currentState.whoseTurn, (WORKER,))
            myant3 = getAntList(currentState, currentState.whoseTurn, (DRONE,))
            myant4 = getAntList(currentState, currentState.whoseTurn, (SOLDIER,))
            myant5 = getAntList(currentState, currentState.whoseTurn, (R_SOLDIER,))
            enant1 = getAntList(currentState, 1 - currentState.whoseTurn, (QUEEN,))
            enant2 = getAntList(currentState, 1 - currentState.whoseTurn, (WORKER,))
            enant3 = getAntList(currentState, 1 - currentState.whoseTurn, (DRONE,))
            enant4 = getAntList(currentState, 1 - currentState.whoseTurn, (SOLDIER,))
            enant5 = getAntList(currentState, 1 - currentState.whoseTurn, (R_SOLDIER,))
            foods = getConstrList(currentState, None, (FOOD,))
            homeSpots = getConstrList(currentState, currentState.whoseTurn, (TUNNEL, ANTHILL))
            enSpots = getConstrList(currentState, 1- currentState.whoseTurn, (TUNNEL, ANTHILL))
            grass = getConstrList(currentState, None, (GRASS,))
            

            for ant, index in zip(myant1, range(100)):
                num = index * 2
                x, y = ant.coords
                params[num] = x
                params[num + 1] = y
            for ant, index in zip(myant2, range(100)):
                num = 100 + index * 2
                x, y = ant.coords
                params[num] = x
                params[num + 1] = y
            for ant, index in zip(myant3, range(100)):
                num = 100 * 2 + index * 2
                x, y = ant.coords
                params[num] = x
                params[num + 1] = y
            for ant, index in zip(myant4, range(100)):
                num = 100 * 3 + index * 2
                x, y = ant.coords
                params[num] = x
                params[num + 1] = y
            for ant, index in zip(myant5, range(100)):
                num = 100 * 4 + index * 2
                x, y = ant.coords
                params[num] = x
                params[num + 1] = y    

            for ant, index in zip(enant1, range(100)):
                num = index * 2
                x, y = ant.coords
                params[num] = x
                params[num + 1] = y
            for ant, index in zip(enant2, range(100)):
                num = 100 + index * 2
                x, y = ant.coords
                params[num] = x
                params[num + 1] = y
            for ant, index in zip(enant3, range(100)):
                num = 100 * 2 + index * 2 + 500 # extra offset is for player 2
                x, y = ant.coords
                params[num] = x
                params[num + 1] = y
            for ant, index in zip(enant4, range(100)):
                num = 100 * 3 + index * 2 + 500
                x, y = ant.coords
                params[num] = x
                params[num + 1] = y
            for ant, index in zip(enant5, range(100)):
                num = 100 * 4 + index * 2 + 500
                x, y = ant.coords
                params[num] = x
                params[num + 1] = y    

            for food, index in zip(foods, range(100)):
                num = index * 2 + 1000
                x, y = food.coords
                params[num] = x
                params[num + 1] = y
            
            for spot, index in zip(homeSpots, range(100)):
                num = index * 2 + 1100
                x, y = spot.coords
                params[num] = x
                params[num + 1] = y

            for spot, index in zip(enSpots, range(100)):
                num = index * 2 + 1200
                x, y = spot.coords
                params[num] = x
                params[num + 1] = y

            for spot, index in zip(grass, range(100)):
                num = index * 1 + 1300
                x, y = spot.coords
                params[num] = x
                params[num + 1] = y

            params.append(max(0.0, min(1.0, evaluation)))
